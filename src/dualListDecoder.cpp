...
std::vector<double> ComputeSquaredDifferences(const std::vector<double>& vector1, const std::vector<double>& vector2) {
...
double SumGroupIndexElements(const std::vector<double>& inputVector, std::size_t groupLength, std::size_t groupIndex) {
  // Check if the length of the vector is a multiple of group length
  if (inputVector.size() % groupLength != 0) {
      // You can handle this error in your preferred way, e.g., throw an exception
      throw std::invalid_argument("Vector size must be a multiple of group length");
  }
...
DualListDecoder::DualListDecoder(CodeInformation encoder, std::vector<CodeInformation> code_info, int max_searched_path)
    : encoder_(encoder), code_info_(code_info), max_path_to_search_(max_searched_path) {
  CodeInformation code_list_0 = code_info[0];
  CodeInformation code_list_1 = code_info[1];
...
DLDInfo DualListDecoder::AdaptiveDecode_SimpleAlternate_rate_1_2(std::vector<double> received_signal, std::vector<std::chrono::milliseconds>& timeDurations) {
  ...
  */
  ...
  // list decoder 0
  std::vector<std::vector<Cell>> trellis_0 =
      ConstructZTCCTrellis_WithList_EuclideanMetric(received_codec_2, code_0, trellis_ptrs_[0], timeDurations[0]);
  int num_total_stages_0 = trellis_0[0].size();
  std::vector<std::vector<int>> prev_paths_list_0;
  MinHeap* heap_list_0 = new MinHeap;
  DetourNode node_0;
  ...
  // list decoder 1
  std::vector<std::vector<Cell>> trellis_1 =
      ConstructZTCCTrellis_WithList_EuclideanMetric(received_codec_1, code_1, trellis_ptrs_[1], timeDurations[0]);
  int num_total_stages_1 = trellis_1[0].size();
  std::vector<std::vector<int>> prev_paths_list_1;
  MinHeap* heap_list_1 = new MinHeap;
...
DLDInfo DualListDecoder::LookAheadDecode_SimpleAlternate_rate_1_2(
  ...
  */
  ...
  // compute min_add_zero = sum of all last of every three squares of differences
  // compute min_add_one = sum of all first of every three squares of differences
  ...
  //      stop searching list 0 when the next item in the list with list metric, d_0
  //            1. d_0 plus min_add_zero is greater to d_hat, then we stop searching on list 0.
  //               For anything not greater, compute crc... as if you havn't found a match
  //      Need to determine when to stop searching list 1
  //      stop searching list 1 when the next item in the list with list metric, d_1
  //            1. d_1 plus min_add_one is greater to d_hat, then we stop searching on list 1.
  //               For anything not greater, compute crc... as if you havn't found a match

  //      if a better match is found, reset d_hat. and continue the search on both lists.
  //   Result: unless we reach the end of the lists, we cannot fail if SSV succeeds.

  // Hard Decode
  std::vector<double> hard_decoding_result = HardDecode(received_signal);
  
  // Compute the minimum possible squared difference
  std::vector<double> squared_differences = ComputeSquaredDifferences(hard_decoding_result, received_signal);
  
  // Once the latest metric on list decoder 0 plus min_add_zero is greater than the best match, stop searching on list 0
  ...
  // list decoder 0
  std::vector<std::vector<Cell>> trellis_0 =
      ConstructZTCCTrellis_WithList_EuclideanMetric(received_codec_2, code_0, trellis_ptrs_[0], timeDurations[0]); // using 1st and 2nd
  int num_total_stages_0 = trellis_0[0].size();
  std::vector<std::vector<int>> prev_paths_list_0;
  MinHeap* heap_list_0 = new MinHeap;
  DetourNode node_0;
  ...
  while (!best_combined_found) {
    ...
    if (!decoder_0_stop && !decoder_0_LSE) {
      ...
      if (!mi_0.list_size_exceeded) {
        // if list size is not exceeded
        if (mi_0.path_metric + min_add_zero > best_current_match || mi_0.path_metric >= decoder_threshold_0) {
          // std::cout << "Decoder 0 declared stop after crc list size " << mi_0.list_rank << std::endl;
          decoder_0_stop = true;
        }
        // std::cout << "mi_0 path metric before true metric: " << mi_0.path_metric << std::endl;
        // Re-encode process
        ...
        // double true_metric = 0.0;
        // for (int i = 0; i < reencode_encoded_msg.size(); i++) {
        //   true_metric += (reencode_encoded_msg[i] == 0) ? metric_0[i] : metric_1[1];
        // }

        
        // std::cout << "d_hat: " << d_hat << std::endl;
        // std::cout << "true metric: " << true_metric << std::endl;
        
        mi_0.path_metric = d_hat;
        
        // std::cout << "mi_0 path metric after true metric: " << mi_0.path_metric << std::endl;
        // mp.insert(mi_0, true_metric);
        mp.insert(mi_0);
        // psedu code
        // check if there is a match
        
        best_current_match = (best_current_match > d_hat) ? d_hat : best_current_match;
        // std::cout << "current best combined_metric: " << best_current_match << std::endl;
        output_0.push_back(mi_0);
      }
    }
    // list decoder 1 traceback
    ...
    if (!decoder_1_stop && !decoder_1_LSE) {
      ...
      mi_1.decoder_index = 1;
      if (!mi_1.list_size_exceeded) {
        if (mi_1.path_metric + min_add_one > best_current_match || mi_1.path_metric >= decoder_threshold_1) {
          // std::cout << "Decoder 1 declared stop after list size " << mi_1.list_rank << std::endl;
          decoder_1_stop = true;
        }
        // std::cout << "mi_1 path metric before true metric: " << mi_1.path_metric << std::endl;
        // Re-encode process
        ...
        //   true_metric += reencode_encoded_msg[i] == 0 ? metric_0[i] : metric_1[1];
        // }
        // mp.insert(mi_1, true_metric);
        mi_1.path_metric = d_hat;
        // std::cout << "mi_1 path metric after true metric: " << mi_1.path_metric << std::endl;
        mp.insert(mi_1);
        best_current_match = (best_current_match > d_hat) ? d_hat : best_current_match;
        // std::cout << "current best combined_metric: " << best_current_match << std::endl;
        output_1.push_back(mi_1);
    ...
    if (mp.queue_size() != 0) {
      ...
      // if mp.queue.size() hasn't changed since last time, we don't have to reconstruct.

      if (mp_size != mp.queue_size()) {
        // std::cout << "New Agreed message found: " << std::endl;
        mp_size = mp.queue_size();
        // std::cout << "size of queue: "  << mp.queue_size() << std::endl;
        DLDInfo agreed_message = mp.get_top();
        // double difference_metric = agreed_message.combined_metric;
        ...
        // mp.insert(agreed_message);

        // if (d_hat < best_current_match) {
        //   l0 = agreed_message.list_ranks[0];
        //   l1 = agreed_message.list_ranks[1];
        //   if (best_current_match < 10000) {
        //     std::cout << "Previous one: " << best_current_match << std::endl;
        //     std::cout << "We found something better" << std::endl;
        //     std::cout << "The better one is " << d_hat << std::endl;
        //     std::cout << "List sizes: " << agreed_message.list_ranks[0]-l0 << ", " << agreed_message.list_ranks[1]-l1 << std::endl;
        //     best_current_match = d_hat;

        //   }
        // }
        // std::cout << "Combined metric: " << agreed_message.combined_metric << std::endl;
        // std::cout << "d_hat: " << d_hat << std::endl;
      }

      // if (output_0.back().path_metric + min_add_zero >= best_current_match) {
      //   //std::cout << "decoder 0 checked out from here" << std::endl;
      //   decoder_0_stop = true;
      // }
      // if (output_1.back().path_metric + min_add_one >= best_current_match) {
      //   // std::cout << "decoder 1 checked out from here" << std::endl;
      //   decoder_1_stop = true;
      // }

      // both decoders have to declare stop of some kind
      //if ((decoder_0_LSE || decoder_0_stop) && (decoder_1_LSE || decoder_1_stop)) {
      if (decoder_0_LSE || decoder_1_LSE || (decoder_0_stop && decoder_1_stop)) {
        // if both lists declare stop AND there is something already in the queue
        // case 1: both declare LSE
        //         WE MIGHT get undetected error
        
        // case 2: one declares LSE, the other one declares STOP
        //         We MIGHT get undetected error

        // case 3: both declare STOP
        //         we can't get undetected error

        // if (decoder_0_LSE) {
        //   std::cout << "decoder 0 LSE" << std::endl;
        // } else if (decoder_1_LSE) {
        //   std::cout << "decoder 0 LSE" << std::endl;
        // } else if (decoder_0_stop && decoder_1_stop) {
        //   std::cout << "flags: " << decoder_0_stop << ", " << decoder_1_stop << std::endl;
        //   std::cout << "stop by verification" << std::endl;
        // }
        ...
        // std::cout << "true metric: " << d_hat << std::endl;
        return agreed_message;
      } else {
        continue;
      }
...
std::vector<double> DualListDecoder::HardDecode(const std::vector<double>& received_signal) {
    ...
    for (const double& value : received_signal) {
        if (value > 0) {
            ...
            // If value is 0, you can decide how to handle it (set to 0.0 or some default value)
